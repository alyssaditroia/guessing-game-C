...
void updateLeaderboard(User leaderboard[], User currentUser) {
  ...
  // Read existing leaderboard and count the number of entries
  int leaderCount = 0;
  ...
  // Close the file to reset the file pointer for rewriting later
  fclose(file);

  // Check if the users score is less than someone on the leaderboard
  ...
  // Insert the new leader into the leaderboard array
  ...
  // Write the leaderboard back to the file
  file = fopen(LEADERBOARD, "w");
  if (file == NULL) {
